...
typedef enum : NSUInteger {
    TYActivatorModeEZ,///< The Wi-Fi Easy Connect (EZ) mode.
    TYActivatorModeAP,///< The access point (AP) mode.
    TYActivatorModeQRCode,///< The QR code mode.
    TYActivatorModeWired, ///< The wired mode.
} TYActivatorMode;

typedef enum : NSUInteger {
    TYActivatorStepFound = 1,///< Scans for a device.
    TYActivatorStepRegisted = 2,///< Registers the device in the cloud.
    TYActivatorStepIntialized = 3,///< Initializes the device parameters.
    TYActivatorStepTimeOut = 4, ///< Sets the timeout value for device pairing.
} TYActivatorStep;
...
/// Get the service set identifier (SSID) of the current Wi-Fi network.
///
...
/// Starts the EZ mode to pair multiple devices.
/// @param ssid The name of the router.
/// @param password The password of the router.
/// @param token The pairing token.
/// @param timeout The timeout value. Unit: seconds. Default value: 100.
